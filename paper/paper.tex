...
that, at every batch, observes a nine-feature ``instrument panel'' of the running
...
slight overshoot the optimal stop. We train and evaluate on QSimBench, using a strict split in which
...
between a user and a quantum-cloud backend. A user submits a circuit $C$ and a
target accuracy $\delta$; the service forwards $C$ to the chosen backend $Q$,
executes it one batch of shots at a time, and after each batch inspects the
updated empirical distribution to decide whether to request another batch or to
stop. On stopping it returns the estimated output distribution together with the
number of shots actually spent. The service never inspects the circuit or the
noise model: from its viewpoint the backend is a black box that, given a batch,
returns measurement outcomes. Its single non-trivial decision, \emph{continue or
stop}, is precisely the stopping policy this paper learns.
...
\begin{tikzpicture}[
  ...
  \node[svc, right=20mm of user] (svc) {\textbf{Shot-allocation service}\\{\scriptsize (gateway)}\\[2pt]{\scriptsize\itshape learned stop/continue}\\{\scriptsize\itshape policy (this work)}};
  ...
  \draw[->] ([yshift=4.5mm]svc.east) -- node[lbl, above]{batch of\\shots} ([yshift=4.5mm]qpu.west);
...
and the number of shots spent. This paper concerns the service's stop/continue
policy, which is \emph{learned} (Fig.~\ref{fig:arch}).}
\label{fig:service}
\end{figure}
